...
api.interceptors.request.use(
    (config) => {
        ...
        // Block any calls to /api/savings or /savings (without /monthly/all) - redirect to monthly/all
        if (config.method?.toLowerCase() === 'get' && 
            (config.url === '/api/savings' || config.url === '/savings' || config.url.endsWith('/savings'))) {
            console.warn('⚠️ BLOCKED: Attempted to call', config.url, '- redirecting to /api/savings/monthly/all');
            config.url = '/api/savings/monthly/all';
            const newFullUrl = config.baseURL + config.url;
            console.log('✅ Redirected to:', newFullUrl);
        }
...
export const savingsAPI = {
    // getAll method removed - only using getAllMonthlySavings for monthly savings list
    // getAll: (params) => api.get('/api/savings', { params }), // REMOVED - use getAllMonthlySavings instead
    getById: (id) => api.get(`/api/savings/${id}`),
    create: (savingsData) => api.post('/api/savings', savingsData),
    // Main method for getting all monthly savings - replaces getAll
