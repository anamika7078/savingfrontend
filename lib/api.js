 ...
 * - It always points to the root of the backend (no trailing `/api` segment)
 * - It has no trailing slash
 *
 * This prevents production issues where `NEXT_PUBLIC_API_URL` is set to
 * `https://backend.com/api` while our routes also start with `/api/...`,
 * which would otherwise result in requests like `/api/api/savings`.
 */
const normaliseBaseUrl = (rawUrl) => {
    if (!rawUrl) return 'https://savings-2-ckrp.onrender.com';
    ...
    // If the URL ends with `/api`, strip that segment so our route
    // definitions (`/api/...`) don't get duplicated.
...
const API_BASE_URL = normaliseBaseUrl(process.env.NEXT_PUBLIC_API_URL || 'https://savings-2-ckrp.onrender.com');
